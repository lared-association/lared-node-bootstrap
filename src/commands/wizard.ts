...
export default class WizardCommand extends Command {
    ...
    static examples = [`$ lared-node-bootstrap wizard`];
...
export class Wizard {
    ...
    public async execute(flags: {
        ...
        this.logger.info('Welcome to the Lared node wizard! This command will:');
        ...
        if (network == Preset.bootstrap) {
            ...
            this.logger.info('');
            this.logger.info(`$ lared-node-bootstrap start -p ${preset} -a ${assembly}`);
            return;
        }
        ...
        if (
            ...
            !(
                await prompt([
                    {
                        name: 'offlineNow',
                        message: `Lared node is about to start working with sensitive information (private keys) so it is highly recommended that you disconnect from the network before continuing. Say YES if you are offline or if you don't care.`,
                        type: 'confirm',
                        default: true,
                    },
                ])
        ...
        this.logger.info('');
        this.logger.info(
            'Lared node needs to provide the node with a number of key pairs (Read more at https://docs.symbolplatform.com/concepts/cryptography.html#symbol-keys).',
        );
        this.logger.info(`If you don't know what a key is used for, let Lared node generate a new one for you.`);
        ...
        this.logger.info('');
        this.logger.info('');
        this.logger.info(`The Lared node preset file '${customPresetFile}' has been created!!!. Keep this safe!`);
        this.logger.info('');
        this.logger.info(`To decrypt the node's private key, run: `);
        this.logger.info('');
        this.logger.info(`$ lared-node-bootstrap decrypt --source ${customPresetFile} --destination plain-custom-preset.yml`);
        this.logger.info('');
        this.logger.info('Remember to delete the plain-custom-preset.yml file after used!!!');

        this.logger.info(
            `You can edit this file to further customize it. Read more https://github.com/symbol/lared-node-bootstrap/blob/main/docs/presetGuides.md`,
        );
        this.logger.info('');
        this.logger.info(
        ...
        this.logger.info(`$ lared-node-bootstrap start -c ${customPresetFile} ${targetParam}`);
        ...
        this.logger.info('');
        this.logger.info(`$ lared-node-bootstrap pack -c ${customPresetFile} ${targetParam}`);
        this.logger.info('');
        this.logger.info(
        ...
        this.logger.info('');
        this.logger.info(`$ lared-node-bootstrap link --useKnownRestGateways -c ${customPresetFile}`);
        this.logger.info('');
    }
