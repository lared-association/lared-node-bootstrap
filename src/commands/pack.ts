...
export default class Pack extends Command {
    ...
    static examples = [
        ...
        `$ lared-node-bootstrap pack -c custom-preset.yml`,
        `$ lared-node-bootstrap pack -p testnet -a dual -c custom-preset.yml`,
        `$ lared-node-bootstrap pack -p mainnet -a dual --password 1234 -c custom-preset.yml`,
    ...
    public async run(): Promise<void> {
        ...
        const targetZip = join(dirname(flags.target), `lared-node-bootstrap.zip`);
        ...
        if (
            ...
            !(
                await prompt([
                    {
                        name: 'offlineNow',
                        message: `Lared node is about to start working with sensitive information (certificates and voting file generation) so it is highly recommended that you disconnect from the network before continuing. Say YES if you are offline or if you don't care.`,
                        type: 'confirm',
                        default: true,
                    },
                ])
        ...
        logger.info(`$ lared-node-bootstrap start`);
    }
}
