...
export class ConfigService {
    ...
    public async run(): Promise<ConfigResult> {
        ...
        try {
            ...
            const oldPresetData = this.configLoader.loadExistingPresetDataIfPreset(target, password);
            if (oldPresetData) {
                // HACK! https://github.com/symbol/lared-node-bootstrap/pull/270 would fix this!
                    ...
                    if (gatewayPreset.restSSLKeyBase64 && gatewayPreset.restSSLCertificateBase64) {
                        ...
                        if (
                            ...
                            throw new KnownError(
                                `Native SSL is enabled but restSSLKeyBase64 or restSSLCertificateBase64 properties are not found in the custom-preset file! Either use 'lared-node-bootstrap wizard' command to fill those properties in the custom-preset or make sure you copy your SSL key and cert files to ${moveTo} folder.`,
                            );
                        } else {
                            this.logger.info(
